...
class Experiment:
    ...
    def __init__(
        ...
        self._maximum_survival_time_steps = int(
            config["experiment"]["maximum_survival_time_steps"]
        )
        self._device = device
        ...
        self._eps_checkpoints = [
            int(v) for v in config["exploration"]["eps_checkpoints"].split(".")
        ]
    ...
    def start(self):
        ...
        for n in range(self._n_experiment):
            ...
            for episode in range(self._n_episode):
                ...
                while not done:
                    print(
                        f"experiment : {n}/{self._n_experiment}, episode : {episode}/{self._n_episode}, exploration: {self._eps}"
                    )
                    ...
                    if done or t >= self._maximum_survival_time_steps:
                        ...
                        t += 1
                print(
                    f"{n}/{self._n_experiment}-th experiment, episodes: {episode}/{self._n_episode}, Score: {t} "
                )
