...
public struct TensorShape
{
    ...
    /// </summary>
    public void GetPositionsFromIndexNCHW(int index, ref int b, ref int h, ref int w, ref int ch)
    {
        w = index % width;
        h = (index / width) % height;
        ch = (index / (width * height)) % channels;
        b = (index / (width * height * channels)) % batch;
    }
    /// <summary>
    ...
    public int IndexWithClamp(int b, int h, int w, int ch)
    {
        ...
        w = Math.Max(w, 0);
        ch = Math.Max(ch, 0);
        b = Math.Min(b, batch - 1);
        h = Math.Min(h, height - 1);
        w = Math.Min(w, width - 1);
        ch = Math.Min(ch, channels - 1);
        return Index(b, h, w, ch);
    }
    /// <summary>
    ...
    /// </summary>
    public int IndexNCHW(int b, int h, int w, int c)
    {
        int index =
...
public class Tensor : IDisposable
{
    ...
    /// <summary>
    /// Create a Tensor of shape [N,H,W,C], an array of data `srcData` and an optional name `n`
    ...
    /// <summary>
    /// Create a Tensor of shape [N,1,1,C], an array of data `srcData` and an optional name `n`
    ...
    public Tensor(TensorShape s, float[] srcData, string n = "")
    {
        ...
        m_TensorOnDevice.Upload(srcData, s, 0);
    ...
    /// <summary>
    /// Create a Tensor of shape [N,H,W,C], associated ComputeBuffer `srcBuffer` filled with tensor values, and an optional name `n`
    ...
    /// <summary>
    /// Create a Tensor of shape [N,1,1,C], associated ComputeBuffer `srcBuffer` filled with tensor values, and an optional name `n`
    ...
    /// <summary>
    /// Create a Tensor of shape [N,H,W,C], a ITensorData `d` and an optional name `n`
    /// `srcData` should be of size b*h*w*ch
    /// </summary>
